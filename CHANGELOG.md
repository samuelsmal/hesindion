...
- **The Patzertabelle's temporary results are applied too.** The five results that last "die nächste Handlung", "für 3 Kampfrunden", "bis zur nächsten Aktion" or "bis sie repariert wird" were stated and left to the player's memory, which is where they were forgotten (owner report: "the same behaviour / help for the other possible outcomes"). *Fuß verdreht* and *Zerrung* add a level of Schmerz that shows on the states strip, in every modifier calculation and in the take-damage screen's before/after, and goes again after the third Kampfrunde — the state detail sheet says where the extra level came from and until when. *Stolpern* puts a −2 row named "Stolpern (Patzer)" on the hero's next combat roll of any kind, AT, PA, AW or FK, and on that one only. *Waffe/Schild beschädigt* costs −2 on AT and PA, −4 on FK, but **only for the piece actually in the hand**, lasts past the end of the fight, is marked in the loadout picker and is cleared by a "Repariert" button in a new "Beschädigte Ausrüstung" section on the hero settings screen. *Ladehemmung* shuts the Fernkampf button for two complete Kampfrunden and says until when; *Zu konzentriert* shuts Parade and Ausweichen until the hero's next own action and says why. Only *Kamerad getroffen* and *Fehlschuss* stay the GM's — both happen on the other side of the table, where the app models nothing — and the first now offers its "Kein solches Ziel: Selbst verletzt" fallback as a button.
- **A screen after "Kampf beenden" for switching the fight's states off.** A fight sets states from half a dozen screens — a failed Sturz check, a Beule, the take-damage screen's Selbstbeherrschung probe, the Beengte-Umgebung toggle — and nothing ever took one off again: the fight ended, the player left, and Liegend went into the next scene with them (owner request). Ending a fight now shows what it left behind: every active Zustand and Status with its level and the rules' own sentence about how it ends, and a control that clears it or steps it down. Every tap applies at once and can be taken back on the same screen; "Fertig" then ends the fight exactly as the button did before. Schmerz and Belastung are shown but not offered — they follow from the life points and the armour — and each says so: Belastung comes from the armour and ends when it comes off, and Schmerz is split by where it came from ("Schmerz II aus Lebenspunkten (14/31 LP) — endet durch Heilung", and, when the Patzertabelle added one, "+1 aus Patzer — endet mit dem Kampf", struck through and marked "endet jetzt", because "Fertig" is what ends it). Where Zäher Hund or the cap at Stufe IV mean the lines do not add up to the level in the heading, the row says which of the two it was. A Patzer's damaged equipment is listed as a reminder, since that is repaired under the hero settings. A fight that left nothing behind ends straight away, with no screen in between.
- **The Patzertabelle asks whether the weapon can be destroyed at all.** "Certain weapons cannot be destroyed. The wizard's staff for example" (owner report). The tables have always printed the rule — "Bei unzerstörbaren Waffen: Waffe verloren" on results 2, 3 and 4 — and the app has always rolled straight past it, because nothing in an Optolith export says which weapons it is about. Those three results now stop and ask: "Ist der Magierstab unzerstörbar?" Until it is answered nothing is written and the screen offers no way on, as with an unrolled Sturz check. **Nein** destroys or dents it as before. **Ja** applies the table's own result 5 instead — the thing is on the floor, not broken — and says so: "Magierstab ist unzerstörbar: „Waffe verloren" statt „Waffe zerstört"". The answer is remembered for the hero, so the same staff is asked about once and is listed under a new "Unzerstörbare Ausrüstung" section on the hero settings screen, where a "Zerstörbar" button takes it back. A "Nein" is not remembered.
...
- **Every weapon shows what the rules say about it.** An ⓘ on the weapon and shield rows of the hero sheet and the loadout picker opens the weapon's entry in Optolith's own inventory: TP, AT/PA-Mod, Reichweite, "geweiht (Boron)" where it is, Waffenvorteil, Waffennachteil and Hinweis — whatever the entry has — next to the hero's own values. A weapon the inventory does not know shows only the hero's values. The import now keeps each weapon's Optolith template id, since names are not unique in Optolith (two Rabenschnabel templates); heroes imported earlier fall back to the name.
- **A weapon the rules list as "geweiht (…)" counts as consecrated** (owner report: "the Rabenschnabel is a consecrated weapon"). It is read by the weapon's name across the inventory, because the Regelwiki is the authority and has one Rabenschnabel, Boron's — Optolith's duplicate template drops the note. That is the default now; the hero settings toggle still switches it off, and still switches any other weapon on. The two answers are stored as overrides of the inventory (`Hero.consecratedWeapons`, `Hero.unconsecratedWeapons`).
- **A defence screen asks who is attacking.** Parieren and Ausweichen on the combat root used to roll straight from whatever the *previous* attack had answered — "even Boronmir when fighting mounted might defend against other mounted opponents" (owner report). Both buttons now open a screen of their own: Vorteilhafte Position, Angriff von hinten, and — when the hero is mounted — whether the attacker is on foot, each with the resulting VW shown in the calculation before the roll.
- **Größenkategorie.** Winzig is −4 AT; groß allows only a shield parry or Ausweichen; riesig only Ausweichen (owner: "this should also block certain actions accordingly"). The opponent-size row now offers all five sizes on both the announcement and the new defence screen, and the weapon list and defence screen hide a parry the size forbids, offering "Stattdessen ausweichen" instead.
- **A Beritten/Zu Fuß switch on the combat root, changeable mid-fight** (owner: "starting the fight mounted does not mean that you are still mounted"). The chip appears beside Beengte Umgebung whenever the hero has a mount, persists with the session, and drives every later roll — Golgariten-Stil's bonus follows the switch, not whatever the hero was riding when the fight began. A hero who ends up Liegend is taken out of the saddle automatically.
- **Kampf im Wasser.** A WASSER chip row (—/hüfthoch/unter Wasser) on the combat root costs AT and PA −2 waist-deep and −6 under water, shuts the Fernkampf button under water, and is eased by Kampf im Wasser (SA_163, −2) or removed entirely by Unterwasserkampf (SA_418) and Wasserlebewesen (ADV_71).
- **Passierschlag from the manoeuvre list, with the hero's own modifiers.** It used to be reachable only from a critical parry, at a bare AT −4; the Manöver list on the announcement screen now always offers it too, and the roll carries every modifier the catalog applies to a melee attack, not just the −4.
- **Blutend.** A Selbstbeherrschung probe sets the clock (7 − QS Kampfrunden, doubled on a Patzer, ended at once by a critical success); while it runs, the round's STATUS panel takes 1 SP at the end of every Kampfrunde (the existing "next round" button, no new round UI) and offers Heilkunde Wunden +2 to shorten it by QS/2 Kampfrunden, rounded up.
- **Rabenschnabel: Dornenspitze and its Waffennachteil.** Its RS −2 against RS 6 or more is offered as a toggle on the announcement and shown as an opponent-RS note, never folded into the opponent's VW total; +1 TP applies automatically when the hero is mounted; and a confirmed attack Patzer with it in hand adds a level of Betäubung, per the weapon's own printed Waffennachteil.
...
- **The mount is always the same animal.** Which pet the hero rides was read as `pets.first` — a SwiftData relationship with no order — so a hero with two animals could ride the horse in one fight and the mule in the next, with the mount's LP bar, its GS in the Sturmangriff damage and its attacks all following that. `Hero.mount` is the first rideable animal by name, and a pet with no initiative (a cat, a mule) is skipped rather than hiding the mount behind it.

- **Weapons, shields and armour are listed in a fixed order.** A SwiftData to-many relationship hands its rows back in no order at all, so the loadout picker, the hero sheet's equipment sections and the armour screens could show the same two weapons either way round between launches — and `CombatViewSnapshotTests.testPreparation` failed about half the time on exactly that. They are sorted by name now (`Hero.meleeWeaponsInOrder` and its siblings), with Optolith's template id breaking a tie between two weapons of the same name. Talents, combat techniques, equipment, languages, spells, liturgies and pets are sorted the same way, and the states strip follows the rulebook's own order instead of the store's, so a Zustand keeps its place between launches.

- **Every interaction starts with a fresh opponent.** Reach, Körperbau, Größe, "Dämon" and "Dämon der Gegengottheit" used to be kept for the whole fight, and only the posture (Liegend, Überrascht, Vorteilhafte Position) was cleared per swing. But the next attack is often at somebody else — "What if I attack someone else?" (owner report) — and the app has no way of knowing it is not, so a Lang picked for the spear-carrier in round one went on charging the hero −2 against the dagger in round four. The GEGNER section on the announcement screen now opens with every question unanswered, **and returning to the round's main screen clears it too**: a Parade or ein Ausweichen rolled between two attacks used to be modified by whatever the last announcement had stated about a creature who is no longer the one swinging — for a mounted hero "Gegner kämpft zu Fuß" went on paying +2 Vorteilhafte Position against everyone for the rest of the fight. Both screens are shut or silent by default and most swings answer nothing in them; the second half of a dual attack, the opponent's defence lines and the damage screen still see the opponent the swing was announced against.
...
- **Flucht is a Körperbeherrschung roll** (Kampfmanöver, erschwert by −1 per opponent in reach), not a claimed "Gelungen"/"Misslungen" (owner: "the user can just say that the roll was successful, instead let's do an appropriate Körperbeherrschungs roll"). A failed roll takes the Passierschlag with no defence, routed straight to the take-damage screen; the log now records the roll's real outcome.
- **Vorteilhafte Position gives +2 on every defence, Ausweichen included**, not only Parade (corrected reading of the Regelwiki: "Erleichterungen von jeweils 2 auf Attacke und Verteidigungen").
- **A Handlungsunfähig hero acts only with the GM's leave** (owner report; Regelwiki, Status Handlungsunfähig: "kann keine Aktionen oder Verteidigungen ausführen" — "der Meister kann freie Aktionen erlauben"). The combat root used to show the banner and leave every button live. Angriff, Fernkampf, Zaubern, Flucht, Parieren, Ausweichen, "Ausrüstung wechseln" and finishing a spell being cast now read as shut and, when tapped, ask whether the GM allowed this one; "Abbrechen" costs nothing and the answer is not remembered, because permission is given per action at the table. The Schicksalspunkt **"Zustand ignorieren" buys the whole round back** where a Zustand carries the status (a Stufe IV, or eight levels in sum), the same Schip that already ignores their modifiers — a Status that carries it instead, Bewusstlos or Versteinert, is not something a Schip ignores. Schaden nehmen, the round counter and Kampf beenden are not actions and never ask.
- The mounted-charge manoeuvre is labelled **"Sturmangriff zu Pferd"** in German, so it is no longer mistaken for the Sonderfertigkeit Sturmangriff (SA_62) — Boronmir's own manoeuvre is Berittener Kampf (SA_43), and only the German label was wrong.
...
- **The opponent's defence total on the deal-damage screen read "Parieren", not VW.** The same catalog lines (Liegend, now also Angriff von hinten) apply to an opponent who only dodges, so the label was already corrected on the announcement box but not on `CombatOpponentDefenseView`'s own total. It reads "VW ±N" now, in both places.
- **Looking at the Stolpern penalty spent it.** "Stolpern" and "Zu konzentriert" were consumed when the roll screen opened, and that screen has a back button: opening Parieren, reading the −2 and going back again cost the hero the penalty without a single die being rolled. Both are now spent by the roll itself — the −2 still shows in the calculation afterwards, and a Schicksalspunkt reroll of the same roll neither loses it nor pays for it twice.
- **"Zu konzentriert" could shut off Parade and Ausweichen for the rest of the fight.** It lasts "bis zur nächsten Aktion", and an archer who spends two rounds reloading never takes one, so the two buttons stayed dark forever. A new Kampfrunde lifts it as well.
- **Re-rolling initiative gave a Zerrung eight more rounds.** "Neuer Kampf" sets the round counter back to 1, but the Patzer clocks — the temporary Schmerz and the Ladehemmung — count in absolute round numbers, so one rolled in round 6 still said "bis Ende KR 8" in a count that had just restarted. They are rebased onto the new count now: what was left is kept, what had already run out is cleared.
- **A defence Patzer of 11 or 12 made the opponent’s blow disappear.** The fumble hurt the hero with their own weapon and handed those TP to the take-damage screen prefilled — which is the same screen the blow that got through the failed parry needed, so entering one meant losing the other. The screen now leads on to a second, empty entry titled "Treffer des Gegners" once the self-injury is confirmed.
- **The loadout picker promised a ranged weapon a −2 it charges −4 for.** A damaged bow or crossbow costs −4 on FK (Fernkampf-Patzertabelle), and the badge on its row said −2, the melee figure.
- **A prone hero was offered "GS/2 = 0 Schritt" as a retreat.** Liegend puts the Geschwindigkeit at 1, and half of that read as an arithmetic bug rather than as the rule. The failed flight now says no retreat is possible, with the same note naming Liegend as the source.
- A Beule on a hero already at Betäubung IV reported "Stufe 4 → 4" as though it had written something. It says nothing changed.
- **A Patzer's "Fuß verdreht" reported itself as `state.schmerz.name`.** The Schmerz row on the fumble screen and its log line were built from a string that is not a localization key — Schmerz shares its label with the modifier row (`source.schmerz`) rather than having one of its own — so the raw key was printed where the state's name belongs. It is read off the state catalog now.
- **A hero who parried with the sword while carrying a shield read the Schild-Patzertabelle.** The fumble screen decided which of the two defence tables applied by asking only whether a shield was in the loadout, not what the parry was made with, so a sword parry got the shield's table — and its item results then took the *shield* out of the loadout for a fumble it had no part in. The table now follows the piece the weapon list rolled with.
