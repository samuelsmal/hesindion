...
- `specs/rules/CHAP_Reiterkampf.yaml` — the first chapter page authored, against <https://dsa.ulisses-regelwiki.de/Reiterkampf.html> with a verified `source.hash`. Two mechanics the app has implemented in Swift since before the corpus existed are now data: the mounted BE relief (`Hesindion/Engine/SharedModifiers.swift:15`) and the mounted `-2` on Ausweichen (`Hesindion/Engine/DefenseModifiers.swift:47-53`). Its other seven clauses — the mount-control probes, the fall check and its per-5-SP escalation, the Passierschlag targeting restriction, the maneuver and equipment bars, the three mounted commands — are `reminder` rows with `UNENCODED:` notes, because a chapter rule has no `rules_i18n` text to degrade to and silence is the failure mode ADR-0008 exists to prevent. No Swift was changed
