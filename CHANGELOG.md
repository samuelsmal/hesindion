...
- **Open question, unruled: `SharedModifiers.encumbrance` applies the mounted BE relief to `spellCasting` and `liturgyCasting` as well as to the three melee domains and ranged attack.** The clause scopes it to Kampfproben, and a Zauberprobe is not one — so `CHAP_Reiterkampf` authors the row as `scope: combat` and the Swift looks over-applied. It changes numbers at the table for any mounted caster, nobody has ruled on the reading, and the Swift is deliberately left as it is. This is the same seam as ADR-0008's last consequence about `CheckDomain` having no domain for INI or GS, in the other direction
