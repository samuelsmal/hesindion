...
- Agents run against a **sanitised workspace** holding the schema, the vocabulary registry, the ADRs and the rest of the corpus, but not the authored file for any rule in the run. Found end-to-end rather than in theory: given the repo as their working directory, both agents read `specs/rules/SA_65.yaml` and reproduced the hand-authored file down to the wording of every note, which would have let Task 7's calibration score a copy as a pass
