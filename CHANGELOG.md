...
- **Every Sonderfertigkeit the app handles is now covered by a test.** The ids were typed inline at a dozen call sites, and an ability can go missing in three ways that all look identical on screen — the ability is on the hero sheet, and the roll is merely a little low. They live in one `CombatAbility` enum, checked against `rules.db` by name; each records what actually reaches it — all eleven by hand, because `RuleEffectModifiers` is not wired into the engine and nothing calls it (issue #27) — and importing a hero carrying an ability nothing implements fails a test rather than going unnoticed at the table
...
- **The opponent's Rüstungsschutz, entered by hand and kept nowhere.** Two rules double the Trefferpunkte *before* armour is subtracted — a critical and a consecrated weapon against its opposing deity — and with no RS on the screen there was nothing for "before" to mean. 5 + 4, doubled twice, is 36 TP; less an RS of 4 it is 32 LP, where subtracting first would have given 26. Not persisted: the RS of the thing in front of the hero is the GM's to hand over or to withhold
