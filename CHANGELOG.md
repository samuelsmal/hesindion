...
- **A Handlungsunfähig hero can no longer act or defend** (owner report; Regelwiki, Status Handlungsunfähig: "kann keine Aktionen oder Verteidigungen ausführen"). The combat root used to show the banner and leave every button live. Angriff, Fernkampf, Zaubern, Flucht, Parieren, Ausweichen, "Ausrüstung wechseln", finishing a spell being cast and the Schip defence bonus are now shut while the status lasts, with one line under the actions and one under the defences saying why. Schaden nehmen, the Schip "Zustand ignorieren", the round counter and Kampf beenden stay open.
