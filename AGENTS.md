...
- **The authoring pipeline is calibrated, and it does not yet pass.** `tests/rules/test_calibration.py` grades a recorded live run over the ten golden rules: **7 of 10** on the mechanical rubric (effect type, target, value, tier, conditions, side, recipient, parameter, vocabulary token). Two of the three failures are one defect: **the agents are handed the Optolith seed text from `rules.db`, not the Regelwiki text the corpus is authored from**, so where the seed is stale the pipeline is confidently wrong and the second agent — reading the same input — agrees with it. Fixing that means resolving each rule's text from its `source.url`. Until then, treat a proposal for a rule whose seed entry looks thin as suspect, and read the page before accepting it. The third failure is a modelling divergence, not an error, and is recorded as such.
