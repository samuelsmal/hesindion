...
- **`RuleEffectModifiers` is the default path.** An ability that is a flat number against a named value needs no code — add the row to `rules.db`. Hand-wiring is for the rules that are not addends: a *choice* between two bonuses (Plänkler-Formation), a penalty that is halved rather than shifted (Gezielter Angriff), a style that pays out only in one loadout (Golgariten-Stil)
