...
- **Activation is per rule**: `FokusRule` (`Hesindion/Models/FokusRule.swift`) lists the optional Fokus-Regeln; `Hero.activeCombatFokusRules: [String]` stores which are on for the current combat, via `isFokusRuleActive(_:)` / `setFokusRule(_:active:)`. All off by default. `CombatFokusRulesSection` renders one toggle per `FokusRule.allCases` on the **armour-selection** step — the unconditional first step of every combat (the combat *setup* step only appears for a Plänkler-Formation or a mount) — so a new rule costs one enum case plus two localized strings.
