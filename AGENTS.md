...
- **Multiple defenses**: -3 per defence *already made* this round, cumulative, reset per round. The count is incremented when a defence roll is set up (`CombatExecutionView.onAppear`), not when the button is tapped, so the first defence of a round is unmodified
...
- **What the app cannot know**: no Optolith export carries a weapon's Leiteigenschaft threshold (TP/KK) — there is no equipment table in `rules.db` (issue #14) — so the damage roll has a manual TP modifier for it
