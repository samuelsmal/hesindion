...
- **The rule website is the source of truth.** Where a local copy and <https://dsa.ulisses-regelwiki.de/> disagree, the rule website wins and the local copy is the bug — see **ADR-0007**. One YAML file per rule under `specs/rules/` carries our mechanical encoding plus provenance (`source.url`, `book`, `page`, `checked`, `hash`) and **no rule text** — the Data Policy below applies. `Hesindion/Resources/rules.db` is a **generated, untracked build artifact** (`make rules-db`), never edited by hand. Optolith supplies the rule ID namespace and the initial seed, pinned by `specs/rules/SOURCES.yaml`, nothing more.
- **The corpus covers chapter rules too, not only abilities.** A rule that binds anyone in a situation rather than anyone with an ability — mounted combat, Beengte Umgebung, multiple defences — has no Optolith id, so it is authored under the `CHAP_` namespace as one file per *page*, id derived from its `source.url` stem (`CHAP_Reiterkampf`). `make rules-lint` enforces the derivation. Everything else is identical to an ability file. See the 2026-09-21 amendments to **ADR-0007** and **ADR-0008**, and Task 12 of `docs/plans/2026-09-20-rules-pipeline-and-authoring.md` for the pages still hardcoded in Swift.
- **Every authored rule declares which set it belongs to** — `ruleset: core` (a standard rule of the Regelwerk), `focus.<slug>` (a Fokus-Regel) or `house.<slug>` (a table's own), see **ADR-0009**. The field is required and has no default, because an optional rule nobody marked would apply to every hero and still look correct; a `focus.`/`house.` slug must be glossed in `specs/rules/vocabulary.yaml` like any other open-vocabulary token. Where the page does not state the set, the answer is `core` *plus* an `UNCLEAR-RULESET:` prefix on the rule's root note — guessing in the visible direction is the rule, but a silent guess is still a guess, so `grep -r UNCLEAR-RULESET specs/rules` enumerates every rule whose set was guessed rather than read, exactly as `UNENCODED:` enumerates the encoding debt. The build carries `ruleset` into `rules.db` as a `rules` column and `make rules-db-verify` checks it against the authored files, because the engine reads the database and a field that stops at the YAML applies to nobody. The engine applies only the sets a hero plays with, so `FokusRule` becomes a projection of that registry rather than a hand-maintained Swift enum. ADR-0009 also decides that every engine output carries the id of the rule that produced it, so the UI can explain any number it shows by rendering that rule's own name and text from `rules_i18n` — no English display text, no translation layer, and no relaxation of the Data Policy.
- **Rule identity is ours; Optolith names abilities only.** `CHAP_<PageSlug>` is the first instance of our own namespace, not a side namespace beside Optolith's. The named combat constants and the derived-value rules belong in it too and are still Swift literals — Task 12 of `docs/plans/2026-09-20-rules-pipeline-and-authoring.md` enumerates them with their current file and line.
...
- `scripts/rules_lint/lint.py` (`make rules-lint`) rejects a `text` key anywhere in an authored `specs/rules/*.yaml` file (including nested, e.g. inside a `when` predicate value) and rejects any `#` comment, since both are routes rule prose could otherwise reach git. **Known limit**: it cannot catch prose smuggled into a legitimately free-form string field (`dice.add`, `actionEconomy.grants`/`forbids`, `parameter`, `skill`, `state`, `action`, `attribute`) — there is no way to distinguish a short expression/identifier from a sentence by shape alone. Authors and reviewers are the backstop for that route — plus, for anything the pipeline writes, `propose.py`'s shingle check, which rejects an encoding echoing three consecutive words of its source text.
- `make rules-lint` also rejects an unresolved `DISAGREEMENT:` marker on a rule's root note: that is `propose.py` recording that two independent encodings differed, and it is not an authored rule until a human has chosen between them.
