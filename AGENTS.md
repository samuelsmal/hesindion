...
- **Multipliers act on the Trefferpunkte; armour comes off what they produce.** Karmale Objekte doubles the rolled TP *before* the Rüstungsschutz, and so does a critical, so the opponent's RS is the last row of the damage calculation. It is entered by hand and kept nowhere — the RS of the thing in front of the hero is the GM's to hand over or withhold, and a value carried between fights would be a claim the app is not entitled to make
- **A penalty on the opponent is not a bonus for the hero.** Liegend is −2 on *their* defence and the rules give the attacker nothing for it; a Finte the same. They go in `OpponentProfile.defenseModifiers` and are printed as their own calculation — on the announcement and again on the damage screen, which is where the GM needs the figure
...
- **Sonderfertigkeit ids live in `CombatAbility`, never as bare strings, and every one of them is covered by a test.** An ability can be ignored silently in three ways — a wrong id, the importer filing it in `generalSpecialAbilities` (which it does whenever `rules.db` has no combat-scoped effect, as for SA_884/SA_160/SA_161), or nothing implementing it at all — and none of them show on screen: the ability is on the hero sheet and the roll is merely a little low. So: `Hero.specialAbility(_:)` searches **both** lists, `CombatAbility.wiring` records whether the effects table drives it or the app wires it by hand *and why it cannot be a table row*, and `CombatAbilityCoverageTests` fails if the sample hero carries an ability nothing handles
- **Every ability is hand-wired today.** `RuleEffectModifiers` would build modifier definitions off the `effects` table, but it is not in `ModifierEngine.shared` and nothing calls it — and that table covers 26 of the 2675 rules in `rules.db`, ten of the 1557 Sonderfertigkeiten. `CombatAbility.wiring` says what actually reaches each one, and the coverage test puts a `.fromEffects` claim through the live engine so the field cannot describe a pipeline that does not run. **Issue #27** is where the shape of a general answer is being worked out; until it lands, adding an ability means wiring it and adding it to `CombatAbility`
