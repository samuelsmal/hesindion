...
/// at a time); `RULES_FILES=none` skips the harness; unset or `all` runs every file.
final class SituationsHarnessTests: XCTestCase {
    func testEverySituation() throws {
        ...
        let known = Set(all.situations.map(\.id))
        report.conflictsUnknown = listed.filter { !known.contains($0) }
        var failures: [(CompiledSituation, [Mismatch])] = []
        for s in all.situations where filter.keeps(s.file) {
            guard ActionRunner.canRun(s) else {
                report.add(s, .unsupported, [], conflicts: conflicts)
                continue
            }
            let run = Matcher.run(s, engine: engine)
            let mismatches = run.mismatches + ActionRunner.mismatches(s, engine: engine)
            let verdict = Verdict.of(s, mismatches: mismatches, hit: run.openRulingsHit, conflicts: conflicts)
            report.add(s, verdict, mismatches, conflicts: conflicts)
