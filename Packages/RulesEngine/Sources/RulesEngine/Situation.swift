...
extension Situation {
    ...
    /// - `process.<id>`: the running process's progress, 0 when none runs (the situation lists
    ///   every running process), owner derived;
    ...
    func stateFact(_ name: String) -> FactUse? {
        ...
        let process = "process."
        if name.hasPrefix(process), name.count > process.count {
