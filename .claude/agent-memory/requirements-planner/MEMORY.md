...
## Project Structure
- Xcode project uses `PBXFileSystemSynchronizedRootGroup` -- files added to `Hesindion/` are auto-discovered; no pbxproj editing needed
...
- Entry point: `HesindionApp.swift` sets up `ModelContainer` with schema array
...
## Key Files
- `/Hesindion/HesindionApp.swift` -- app entry, ModelContainer setup
- `/Hesindion/ContentView.swift` -- current root view (template, to be replaced)
- `/Hesindion/Item.swift` -- template model (to be replaced)
