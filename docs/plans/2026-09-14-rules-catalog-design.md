...
**2. Source data.** Optolith carries no mechanics. Its structured fields are cost, prerequisites, levels, `combatTechniques` (which weapons an SA applies with, 215 SAs) and `extended` (which abilities a style unlocks). Its prose has a `penalty` string on 65 of the 226 combat SAs. The 82-row `effects` table was hand-authored in `specs/data/rules.yaml`; the Regelwiki scraper only ever had a hardcoded fallback for six rules. Nothing generates mechanics and nothing can. Every rule's mechanic has to be read, and the Regelwiki is the only complete and authoritative text (Optolith's prose lags: for Golgariten-Stil it says +1 TP where the page says +1 PA, and the app applies both).
