...
- **Orphaned `source.*` strings.** Each move leaves its Swift label unused in `Strings.swift` (`source.multipleDefense`, `source.reach` so far); sweep them once in Task 16 rather than per task.
