...
- [ ] Added before execution: a line from a tiered offer is labelled with its tier — `RuleLine.name` is "Wuchtschlag II" for tier 2 (`CombatManeuver.roman(_:)`), as the Swift `sourceLabel` was — because the tier is the information the row carries and two UI tests (`ComplicatedAttackFlowTests`, `DamageBreakdownFlowTests`) read it. The evaluator appends the roman numeral only when an offer's announced tier is applied; the `ruleId` stays `SA_67`.
...
Elsewhere in AGENTS.md: in the Testing section's list of known intermittent failures, add `DiceRollerTests.testD6IsUniform` beside the D20 one (the same unseeded chi-square; it fired once during Task 12, 16.83 against a 16.75 critical value). In the Testing section's list of accessibility identifiers add `combat.opponent.onFoot` (the announcement's "fights on foot" toggle, Task 10) and `combat.execution.breakdown` (the execution screen's calculation box, which the UI tests scope their row assertions to). In the Trefferzonen section replace "`HitZoneModifiers` emits the Zonenaufschlag as a `ModifierLine` (`SA_160` halves in melee, `SA_161` at range; `targetIsSurprised` is a GM flag on `ModifierContext`, …)" with "`GRW_zonenaufschlag` in the catalog is the Zonenaufschlag; `SA_160` and `SA_161` halve it and an überraschter opponent (`STATE_13`, stated on the roster entry) eases it by 2; `HitZoneModifiers.penalty` is the same table for the zone picker's chips until step 3 reads them off the evaluation". In the "Weapon reach" bullet replace "passed to the engine as `ModifierContext.attackerReach`" with "`Situation.loadoutName` names it and `GRW_reichweite` reads `Situation.loadoutReach`". In the "Karmale Objekte" bullet replace "(`KarmalWeapon`)" with "(`GRW_karmaleObjekte`, read through `DamageModifiers.multiplier(situation:)`; what the screen says about a demon is the toggle's subtitle)". In the "`CombatSituation`" bullet append "It is the round part of `Situation`." In the "Player States" section replace "active Zustände feed penalties into the `ModifierEngine`" with "active Zustände feed penalties into the `ModifierEngine` from `StateModifiers` (`mechanic: .penalty`) or from the catalog (`mechanic: .catalog`, Liegend so far)".
...
**Resolved by the closing sweep (commits `a6d0d83`, `5241988`), kept below for the record:** the dead `sourceLabel` Wuchtschlag branch and the orphaned `source.*` / `modifier.trefferzone` strings, `Hero.hasGolgaritenStil`, the unused `isCriticalHit` parameter, `DamageModifiers.rules` in the allow-list test, the `tp`-multiplier guard counting effects, the wound panel reading the hero's own Selbstbeherrschung id, the `probeTimeout` wait and the duplicated UI-test parry navigation. Still open by decision: `KarmalWeapon.statesSomething` / `Target` / `daemon.regularDamage` (the owner's call), and everything marked step 3.
...
- **An offer has no span.** SA_884's `choice` is decided once per fight on the setup screen, but the clause cannot say so, and the evaluator lists the offer on every roll until it is taken. Step 3 must not build a per-swing manoeuvre control from it; design §4's offer attributes (`excludes`, `cost`, `restrictions`, `outcome`) are where a `decidedAt: setup | announcement | round` belongs when the vocabulary grows.
- **`wrongDomain` also covers "the other option was chosen".** A Plänkler who chose AT sees the rule as `wrongDomain` on a parry. Step 3 words the reason so it covers both, or the evaluator gains an `otherOptionChosen` reason.
- **What Task 12 left dead in `CombatManeuver` is the `.wuchtschlag` branch of `sourceLabel`** (and with it `L("source.wuchtschlag")`): the AT line now comes from the catalog, and `MeleeModifiers.maneuverAT` returns nil for a Wuchtschlag. `atModifier` and `damageBonus` stay — they are the manoeuvre picker's chips (`CombatAttackViews` around the maneuver grid), and `RuleFixtureTests.testTheManoeuvrePickerAndTheCatalogAgreeOnWuchtschlag` holds them to SA_67. The Task 16 sweep deletes the branch and the string, not the two properties.
- **`DamageModifiers.rules` is outside the allow-list test** (`testEveryRuleADefinitionNamesExistsInTheCatalogOrIsStillToBeAuthored` walks `ModifierEngine.shared.definitions` only); include it when that test is next touched.
- **The prone opponent's row is now "Liegend", not "Ziel liegt"** (Task 13): an `opponentAdd` line carries the rule's name, so the hero's own Liegend row and the opponent's share a word, told apart only by the box they sit in. `L("opponent.prone")` stays for the toggle and the lid. Step 3 decides whether opponent lines get a prefix or their own label field; the CHANGELOG entry in Task 16 names the change. Lesson for the UI-label chore: grep the UI tests for the old Swift label's *text* (`Strings.swift`), not for the rule's name.
- **The opponent's defence box costs a second evaluation per render** (`CombatAttackViews.opponentDefenseLines` evaluates `.meleeAttack` again beside `attackBreakdown`); the fix is `ModifierEngine.evaluate` handing out the `Evaluation` it already made — step 3, with the per-render cost note above. The Finte + catalog concatenation moves into a testable function beside `OpponentProfile.defenseModifiers` then; until then `ComplicatedAttackFlowTests` asserts the box.
- **Catalog lines come after all Swift lines, in catalog-id order**, so Liegend left the state rows' place in the breakdown (no snapshot shows it). Part of step 3's display-rank question.
- **"Zustand ignorieren" also switches off Liegend, a Status.** The catalog entry preserves what `StateModifiers` did (its Schip guard ran for every state); `hasIgnorableZustand` rightly does not offer the Schip for Liegend alone. A ruling for the reviewer of STATE_10: either the status ids join `RuleEvaluator.statesTheSchipCannotIgnore` or the behaviour is confirmed.
- `OpponentProfile.defenseModifiers(maneuver:isCriticalHit:)` never reads `isCriticalHit` (older than this work); drop the parameter in the Task 16 sweep. AGENTS.md's line that Liegend and Finte "go in `OpponentProfile.defenseModifiers`" is stale since Task 13 (only Finte remains), and the `StateModifiers.swift` header can mention `.catalog` — both Task 16.
- **Nothing reads `Evaluation.questions` yet** (Task 14). The "Gegengottheit" toggle is shown by Swift under `weaponIsConsecrated && opponent.isDaemon` (`CombatAttackViews` ~717), a second copy of `GRW_karmaleObjekte`'s `when` that no test ties to it; and because `OpponentProfile.isOfOpposingDeity`'s setter stores nil for off, the fact is never `false` from the UI, so the question is emitted on every render and the fixture's `opposing: false` row is unreachable from the app. Step 3 draws the toggle from the question (then "visible exactly when asked" is true by construction) and decides whether off means "no" or "not stated".
- **`KarmalWeapon.statesSomething` and `KarmalWeapon.Target` have no caller outside their tests** (older than Task 14; the string `daemon.regularDamage` is unused too). Task 14 corrected the texts that said the screen uses them. Whether to wire "regulärer Schaden" to a row or delete the type is the owner's call; listed for the Task 16 sweep as a question, not a deletion.
- **The multiplier row's label is Swift's** (`source.karmal.opposing` in `CombatAttackViews` and `CombatDefenseViews`), and `RuleMultiplier.name` is dropped, unlike opponent lines, which carry the rule's name. `RulesCatalogTests` holds the catalog to one representable `tp` multiplier until step 3 carries the name through (and renames `karmalDamage`).
- **Four evaluations per announcement render** after Tasks 13 and 14: `.meleeAttack` twice (AT lines, opponent lines) and `.damage` twice (lines, multiplier), and `proceed()` repeats them. The smallest hoist is `DamageModifiers` returning lines and multiplier from one evaluation; it belongs with step 3's `ModifierEngine` handing out the `Evaluation`.
- `RulesCatalogTests.testEveryTPMultiplierIsKnownAndAtMostOneRuleMultipliesTP` counts rule ids, so one rule with two clauses that both multiply `tp` passes it and could still yield two multipliers at runtime (a debug `assertionFailure` only). Count the effects, or say so in the test, when a second `tp` multiplier is authored.
- CHANGELOG.md's description of `KarmalWeaponTests` ("the consecrated-weapon rule") is stale since Task 14: the rule's rows are in `RuleFixtureTests`, the file keeps the per-hero setting. Task 16.
- **`TalentProbeModal.isWoundEffectProbe` is a bare Bool with a default of false** (Task 15): a future wound-effect call site that forgets it silently loses DISADV_57. One call site today (`CombatTakeDamageView`), a shared helper builds the probe's `Situation` for the panel and the modal, and a comment at `WoundEffectResolver.probeModifier` points to it. When a second occasion appears, an `enum ProbeOccasion { case freeStanding, woundEffect(damage:wundschwelle:) }` that also derives `initialModifier` makes the two inseparable.
- The wound panel asks the engine about `Talent.selbstbeherrschungRuleId` while the modal uses the hero's Selbstbeherrschung row's own `ruleId` (found by name); a hand-built row with another id would show −2 on the panel and not roll it. Have the panel pass `hero.selbstbeherrschung`'s id in the sweep. The panel's format `"Probe: %@ %d"` prints no plus sign; no talent-check rule is positive today — `%+d` when one arrives. One more evaluation per panel render joins the per-render cost note.
- DISADV_57's source page (US25003 p.133) could not be checked against `rules.db` (the `sources` table is empty, `rules` has no page column); the person who sets `reviewed` checks it.
- The design doc (`docs/plans/2026-09-14-rules-catalog-design.md`) still lists Verweichlicht as not honoured (~line 17) and calls its probe a "non-combat flow" (~line 194); it is the in-combat Wundeffekt probe. Task 16 corrects both lines.
- **From the final review of the whole branch:** a `choice` that is not the sole effect of an `offer` clause validates and is then silently ignored by the evaluator, and two offer clauses of one rule in the same domain append the same `RuleOffer` twice (questions are deduplicated, offers are not) — reject the first in `catalog.py` and dedupe the second when step 3 starts reading offers. `catalog_meta`'s hashes cover the YAML and the vocabulary only, so a change to `catalog.py`'s normalisation without a rebuild goes unnoticed; hash the builder too, or have CI rebuild and diff. The literal labels "AT/PA +2" (advantageous-position toggles; a Golgarit actually gets +4 AT), "Parieren −2" (prone toggle) and "TP ×2" (opposing-deity toggle) are prose not tied to the catalog; step 3 draws them from the evaluation. The final review's other findings (the defence button's `pendingMultipleDefensePenalty` disagreeing with Vinsalt-Stil's roll, an unbounded factor, a negative announced tier, the untied ranged zone chip) were fixed before the push.
- ~~UI-test parry navigation lives twice~~ — done in the closing sweep (`UITestSupport.tapParry`). Breakdown rows get their own identifiers (`combat.breakdown.row.<ruleId>`) in step 3 so UI assertions stop matching bare values.
...
- **Orphaned `source.*` strings.** Each move leaves its Swift label unused in `Strings.swift` (`source.multipleDefense`, `source.reach`, `source.vorteilhaft`, `source.golgariten`, `source.plaenkler`, `source.wuchtschlag` so far; `Hero.hasGolgaritenStil` is dead too); sweep them once in Task 16 rather than per task.
