...
- **Orphaned `source.*` strings.** Each move leaves its Swift label unused in `Strings.swift` (`source.multipleDefense`, `source.reach`, `source.vorteilhaft`, `source.golgariten`, `source.plaenkler` so far; `Hero.hasGolgaritenStil` is dead too); sweep them once in Task 16 rather than per task.
