...
- [ ] Order of application: base adds, then `modifyRule` (per line: `set`, then `multiply`, then `add`), then zero lines are dropped as `netZero`. Multipliers and opponent lines are collected beside. The −5 cap is not applied here.
