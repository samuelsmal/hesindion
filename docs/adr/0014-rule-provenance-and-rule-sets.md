# ADR-0014: Rule Provenance at Runtime, and Rule Sets as Data

## Status

Superseded by ADR-0015. Accepted as ADR-0009 on `feat/rules-data-pipeline` (2026-09-21); imported for the rules rework, which reviewed it before anything was built on it. Paths under `specs/rules/`, `scripts/rules_*` and `tests/rules/` live in the tag `archive/rules-data-pipeline`, not on this branch.
...
- **Two sources disagreeing.** `SA_41` existed twice in the legacy stores with different `scope`
  values, which is ADR-0012's founding bug still producing new instances.
...
hero's species sets their base LP. ADR-0012's amendment introduced `CHAP_<PageSlug>` for chapter
...
`resolve(context) -> EngineResult` (ADR-0013) returns `lines`, `dice`, `parameters`,
...
The Swift change belongs to the engine rewrite (ADR-0013), not to the commit that carries this ADR.
...
ADR-0012's founding defect re-introduced at the presentation layer, and it would cost 232 hand-written
strings to acquire.
...
(ADR-0012), and authored files still carry no `text:` key and no German at all — `note` stays ASCII
...
entry behind them. ADR-0013's amendment recorded that as a consequence for a future consumer; with
...
English, ours, and already load-bearing under ADR-0013's amendment. It renders no rule text, because
there is none in git and there never will be.

The alternative — have the build fetch the page and take the title from it — is rejected: ADR-0012
...
should not be there. A rule wrongly marked `focus.` fires for nobody and nothing says so. ADR-0013's
...
page says 250 and carries two cap clauses the seed is missing. ADR-0012 makes the page normative, so
...
- **The core-rule constants ADR-0013 turns into named parameters** — the multiple-defence `−3`, the
...
Task 12 of `docs/rules-rework/2026-09-20-rules-pipeline-and-authoring.md` grows to enumerate these with the
...
- **Author an English display name and summary for every rule so the UI has something to show.**
  ...
  the page, with nothing comparing them — ADR-0012's founding defect rebuilt at the presentation
  layer. The database already ships the German text; rendering it costs nothing and cannot drift.
- **Fetch each chapter page's title at database build time.** Rejected. ADR-0012 requires the build
...
- **A separate manifest listing which rules belong to which set.** Rejected for ADR-0012's reason:
...
it. ADR-0013's note of the same date was given this treatment in the first pass of the review and
...
Same reason as ADR-0013's note of the same date, and the sharper case of the two. Two of the three
...
- **ADR-0012** — the rule website is normative, `rules.db` is a generated artifact, and the
  `CHAP_` namespace this ADR generalises.
- **ADR-0013** — the effect schema, `EngineResult`, and the named-parameter vocabulary that the
