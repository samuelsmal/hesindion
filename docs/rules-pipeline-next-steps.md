...
This document exists to be read before deciding what to spend next. `docs/rules-pipeline-status.md`
...
- **Blocker 1 is not closed.** Its *resolution* half is built; its *backfill* half is not. 31 rules
  ...
  ADR-0007's second 2026-09-21 amendment — and `propose.py` still reads `rules_i18n.description`,
...
**For.** One measurement instead of two, against the input ADR-0007 actually mandates. The gate's own
...
`docs/rules-pipeline-status.md` §3 records that a ten-rule pass on page text scored 6/10, *below* the
