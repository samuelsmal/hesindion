...
| | |
|---|---|
| Decisions | `docs/adr/0012`, `0013`, `0014` — amended, never rewritten |
| Tasks, with status and commit range | `docs/rules-rework/2026-09-20-rules-pipeline-and-authoring.md` |
...
- **`SA_661` — input-caused.** The driver fed the agent the Optolith seed text, which states the
  ...
  reading, per ADR-0012, and its note records the disagreement.
...
`rules.db` — the Optolith seed — while ADR-0012 makes the rule website normative and the golden ten
...
This is not new scope. ADR-0012's Decision already demotes Optolith to a seed that after bootstrap
...
- `docs/adr/0013-rules-as-data-combat-engine.md`'s `CheckDomain` consequence survives redaction as a
...
1. **Resolve each rule's text from its `source.url`, not from `rules.db`.** **Resolution built;
   ...
   *What is still open, and why this blocker stays open.* Three things. **(a)** The 31 rules the
   ...
   claim that a specific page was fetched and normalised, and ADR-0012's second 2026-09-21 amendment
...
An unconditional legality row unlocks mounted-combat actions for a hero on foot. That is ADR-0013's
...
(ADR-0013), so an authored file and a wrong constant can sit beside each other indefinitely without
...
**(a) Ruleset versioning — deferred, with a worked example.** ADR-0014 decides "sets now, versions
...
the live page says 250 and carries two cap clauses the seed lacks. ADR-0012 makes the page normative,
...
reads it.** ADR-0014 says a chapter rule's page title goes in "the chapter rule's `rules` row as its
...
produced ADR-0012's and ADR-0013's 2026-09-21 amendments: a reviewer concludes "there is no clause"
...
the CHANGELOG and ADR-0013's amendment. A note that states a *reason* can be wrong even when the
...
reduction. Settled in the data's favour, `030230e`, and recorded in ADR-0013's amendment. The
...
Recorded in ADR-0013's Consequences as a binding input to the engine plan rather than settled by
...
re-open a resolved one. That is the accepted position, recorded in ADR-0012's 2026-09-21 amendment —
...
Then read, in this order: this file, `docs/rules-rework/2026-09-20-rules-pipeline-and-authoring.md` for what
is done and what is left, and ADR-0012/0013/0014 for why. `CHANGELOG.md` under `[Unreleased]` carries
