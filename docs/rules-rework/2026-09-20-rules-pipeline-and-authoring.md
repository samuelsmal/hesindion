...
> `docs/rules-rework/rules-pipeline-status.md` for what is true now.
>
> **Before running anything, read `docs/rules-rework/rules-pipeline-status.md`.** The calibration gate (Task 7)
...
**Spec:** `docs/adr/0012-rule-website-as-single-rule-source.md` and
`docs/adr/0013-rules-as-data-combat-engine.md`
...
terminology correction `877cb6c`, the combat-scoped mounted relief `030230e`, ADR-0014 and rule sets
...
The **Swift half** of ADR-0013 (`EngineResult`, the effect handlers, `DamageExpression`, maneuver
...
assertion in `HesindionTests/HeroImportTests.swift`), listed in `docs/rules-rework/rules-pipeline-status.md` §7.
...
/// hand-written an effect for it. See ADR-0013.
...
```swift
// Combat SAs are classified from the ruleset's own grouping (ADR-0013), so Plänkler-Formation
...
`forbids` beside `grants`, and a ninth effect type `recovery` was added — ADR-0013 was amended to
...
`docs/rules-rework/rules-pipeline-status.md` §7.
...
- [ ] **Step 1: Write the schema** — `specs/rules/schema.json` (draft 2020-12, abridged to its
      distinctive parts; the eight effect types and ten predicates of ADR-0013 are closed enums)
...
`CheckDomain` has no domain for INI or GS at all — recorded in ADR-0013's Consequences (`faabe52`)
...
1. **An eleventh `when` predicate, `gmFlag`.** Ten legacy rows carry a free-text `condition`; only
   ...
   **unconditional**, which is worse than today's behaviour: `SA_22` would grant +1 Gassenwissen
   ...
   Add it to `specs/rules/schema.json` and to ADR-0013's predicate list.
...
vindicating ADR-0012's premise, and the pass also found the first of this branch's two live app bugs
...
- [~] **SUPERSEDED by the evidence.** `SA_43`'s BE effect carries `when: [{mounted: true}]` — the
      ...
      outrank the evidence. See `docs/rules-rework/rules-pipeline-status.md` §8 — the first reason given for the
...
hand-authored file and returned it byte-identical. See `docs/rules-rework/rules-pipeline-status.md` §2 — this is
...
recorded as an amendment to ADR-0012.
...
by the driver feeding agents the Optolith seed text while ADR-0012 makes the rule website normative.
**The full result, the rubric, the five blockers and the variance datum are in
`docs/rules-rework/rules-pipeline-status.md` §§1–4. Do not start a wave.** Note that criterion 1 below says
...
calibration gate and could be done at any time. Note that ADR-0013's amendment records its known
...
they are enumerated in `docs/rules-rework/rules-pipeline-status.md` §3. At the measured rate a 222-rule wave
...
**Spec coverage.** ADR-0012: single authored store (Tasks 2, 3), no prose in git (Task 2 lint rule),
`rules.db` untracked and generated (Task 1), Optolith pinned (Task 1), two-step sync with the
deterministic half separated (Tasks 5, 6), `HARDCODED_EFFECTS` deleted (Task 3). ADR-0013: the schema
...
> **Answered 2026-09-21 by ADR-0014, and this paragraph conflated two things.** The `ruleset:` key
...
> `SA_62` is the worked example of what that defers; see ADR-0014 and
> `docs/rules-rework/rules-pipeline-status.md` §6a. It stays reversible — a version dimension would extend
...
> started**, so ADR-0013's coverage ratchet is unbuilt. ADR-0013's amendment also records the
...
that a specific page was fetched and normalised (ADR-0012's second 2026-09-21 amendment gives that to
the driver, not to this resolver), and (c) changing `propose.py` to read `source_url` instead of
`rules_i18n.description`. `docs/rules-rework/rules-pipeline-status.md` §3 says which half closed.
...
Optolith seed text, while ADR-0012 makes the rule website normative and specifies the propose step
...
the one commit that last updated this line", and chasing that last commit only moves the gap by one.
Stated once here rather than corrected once per round — if a range ever needs to be exact to the
commit, `git log --oneline <start>..` is the answer, not a fifth edit to this line.)*

**Why it exists:** the whole-branch review's §6 (`docs/rules-rework/rules-pipeline-review-findings.md`), recorded
...
whole encoding is one axis-less row plus one predicate. It also corrected `docs/adr/0013`'s two
passages restating the withheld pair (third repair round on it), and added a dated correction to
`docs/rules-rework/rules-pipeline-review-findings.md` §6 — whose ruling stands, but whose evidence sentence about
...
`docs/rules-rework/rules-pipeline-status.md` §1 and §2 for the cost, and for the one golden rule the closure
...
for the acceptance criteria below, both recorded in `docs/rules-rework/rules-pipeline-status.md` §2:
...
`when: [{mounted: true}]` gate in 2 of 5 passes on identical input, which is ADR-0013's named
...
(`9aef3d2`) and a linter/schema contradiction over chapter ids (`a3508e2`). ADR-0014's two database
...
authoring is recorded and awaits a ruling (`docs/rules-rework/rules-pipeline-status.md` §6b).
...
complete. `CHAP_Reiterkampf` is the first one and establishes the shape (ADR-0012 and ADR-0013,
...
- Modify: `docs/rules-rework/rules-migration-reconciliation.md` (coverage), `CHANGELOG.md`
...
| Dual wield, base penalty | `Hero` (ADR-0013's census) |
| Passierschlag | `passierschlag.penalty` per ADR-0013's parameter list |
...
**Two more categories, added by ADR-0014.** Optolith names abilities and nothing else, so `CHAP_` is
...
*Named combat constants.* ADR-0013 turns these into parameters; each is also a rule that can be
...
| `dualWield.penalty`, `passierschlag.penalty` | `Hesindion/Models/Hero.swift`, per ADR-0013's parameter list |
...
**`source.title` on chapter files (ADR-0014).** A chapter rule has no `rules_i18n` row, so a
...
slug**, which is ADR-0014's argument for admitting the field turned into a check — a description of
...
**`ruleset` reaches `rules.db`, and `rules-db-verify` checks it (ADR-0014).** Also done
...
- [ ] Every file declares its `ruleset` (ADR-0014). A chapter page of the Regelwerk's standard rules
...
- [ ] Every `CHAP_` file carries `source.title`, and `build_db.py` writes it into the chapter rule's
      `rules` row, so a breakdown line citing a chapter rule has a name to display (ADR-0014).
- [ ] Constants that ADR-0013 names as parameters are authored as `parameterOverride` against paths
...
is in `docs/rules-rework/rules-pipeline-status.md` §5.
...
**Still open:** GS has no rule to cite. It is on Task 12's derived-value list and ADR-0014 puts
