...
purpose, per the catalog's note on SA_67, until the loadout can name Raufen (issue #14). Everything
else matches: the catalog implements W1 (`SA_67`, `tiers: owned`), and every Stufe up to the owned
one is offered, so a lower Stufe can be chosen (9.3) — which is what the tiered-manoeuvre ruling
would confirm or undo.

## Rulings

All rulings, open and decided, are in [`RULINGS.md`](./RULINGS.md). This example rests on the same
three cross-cutting ones as Finte.
