...
## Final harness (Task 36, the end of Phase C)

`make test-rules-engine` (`RULES_FILES` unset, which runs every situations file), after Task 36:

`harness: 223 passed, 0 failed, 15 pending, 71 conflict, 0 unsupported` (309 situations)

The engine's done criterion (spec §2: every situation passes, except those resting on an open
ruling) holds except for the 71 conflicts, which wait on the owner's decisions (R60): 0 failed,
0 unsupported. Every pending situation is explained by open rulings its run hit (R32/R41/R48).
Every conflict is listed under "Expectation conflicts for the owner" below, with its reasoning,
in three categories (R77): (a) the expectation wrong per the rule text, 52; (b) the input
convention of R66 (a folded sheet base), 16; (c) rule data not written, 3.
...
Split into three categories by ruling R77. A situation is listed under each category one of
its mismatches belongs to (the Großschild entry in (a) names situations whose parries are in
(b)); counted once, under (b) or (c) when one of its mismatches is of that kind and under (a)
otherwise, the 71 conflicts are (a) 52, (b) 16, (c) 3. Every entry names every mismatch the
harness records for its situations. Those mismatches are recorded as fingerprints (query, step,
kind) in `conflict-fingerprints.json` beside this file (ruling R78): a listed conflict with a
mismatch outside its fingerprints fails the harness, and fingerprints that no longer occur are
printed. After a change to this section or to a listed conflict's mismatches, re-record them with
`make test-rules-engine RECORD_CONFLICT_FINGERPRINTS=1` and review the diff.

### (a) The expectation is wrong per the rule text

The expectation does not follow from the rule text as encoded: a shape no expect key holds (a
check `offered`, a structured text, a comment written as a key), an origin or `via` the
encoding cannot give (a `none` clause, one event with two origins), a value the rules compute
otherwise, or an input the situation does not state (R61). The owner corrects the expectation,
or decides the encoding or ruling the entry names.
...
- situations/boronmir-neu.yaml 19.9: expects `wundschwelle` total 9 with `rulesets: []`. The Wundschwelle's derive (⌈KO/2⌉) is trefferzonen.TZ8's, and the whole file is `ruleset: fokus.trefferzonen`, so with the Fokusregel off only ADV_54's +1 remains. Pending on ADV_54's open ruling eisern-scope, which asks exactly this. Every mismatch the harness records (final review): (a) `wundschwelle` total 9, got 0 with no lines: with `rulesets: []` TZ8's derive is off, and ADV_54.E1's +1 rests on the open ruling eisern-scope, so it applies nothing (§4.7); (b) the expected line carrying `ruling: ADV_54.eisern-scope`: no line carries it, for the same reason; E1 shows its open-ruling text instead ("Die Wundschwelle des Helden steigt durch den Vorteil um 1."); (c) the text `{ player: "wirkt nur mit Fokusregel Trefferzonen", ruling: ADV_54.eisern-scope }`: no clause tells this; it is the answer the open ruling would give, and the text shown for that ruling is E1's own.
...
- situations/probe-magie.yaml 20.7 (Task 35: every remaining mismatch; the entry above names (b)): (a) `offered: [{ choice: split.le, max: 7, from: SA_74.VP1 }]`: VP1's offer rests on the open ruling SA_74.vp-sequence (the original draft put it there, `offer.ruling`), so it applies nothing and shows its clause and question; the offer is missing while the ruling is open (see Open questions, Task 35). (b) the 3 AsP `from: [zaubermodifikationen.ZM12, SA_74.VP1]`, one event with one origin (above). (c) the event `{ check: { talent: TAL_8, with: null }, from: SA_74.VP2, ruling: SA_74.vp-sequence }`: since Task 35 a cast asks the talent checks gated on what it states, but VP2's check rests on vp-sequence (open) and is not asked. The payments (3 AsP and 5 LeP, from VP1), the `after` (AsP 0, LE 24, no Zustand) and TZ8's `notApplied` match. In a throwaway run with vp-sequence and cost-off-table taken as decided, (a) and (c) match and only (b) remains.
- situations/probe-magie.yaml 20.8 (Task 35: every remaining mismatch; the entry above names (a)): (a) the half cost `from: [SA_74.VP3, zaubermodifikationen.ZM12]` (above). (b) the `after` AsP 0 and LE 28: got 3 and 29, nothing is paid: VP3's cost rests on the open ruling vp-sequence (whether a spell check that failed on its own pays this way), and ZM12's cost is suppressed by VP1 once LeP are chosen. (c) `result: { success: false, from: SA_74.VP2, ruling: SA_74.vp-sequence }`, compared since Task 35 (the cast's spell check, run as the step's roll states it): the cast fails, but no clause names the failure, since VP2's check is not asked while vp-sequence is open. In the throwaway run with the rulings decided, (b) and (c) match (the failed Selbstbeherrschung forbids the cast from VP2, resting on vp-sequence; VP3 pays 3 AsP and 1 LeP) and only (a) remains.
...
- situations/probe-fernkampf.yaml 21.4, 21.5b (Task 36: every remaining mismatch; the entry above names it): the one mismatch of each is the structured text `{ result: "hit only on a 1", from: fernkampf.FK9 }` (21.4) / `from: fernkampf.FK10` (21.5b). The pages say it in prose (Regelwerk p. 241 ff., FK9: "Der Schuss auf ein unsichtbares Ziel ist ein reiner Glücksschuss und gelingt nur bei einer 1 auf W20"; FK10: "Tier im Trab | fast unmöglich (Glückstreffer bei einer gewürfelten 1 auf W20)"), and the rules carry it out: FK9's Stufe 4 and FK10's trot cap `fk` at 0 (the `capped` line matches), and the combat roll gives `success: false` on a 3 and `true` on a 1 (21.4's two sequence steps match). What the clause shows is its plain `tell` to the player, "Glückstreffer: der Schuss trifft nur bei einer gewürfelten 1" (FK10: "… im Trab trifft …"). No verb states a roll's result as a text object, and a structured text never matches a clause's plain tells (Task 31 fix round 1, as kupperus-und-waffen 18.1, 18.3, 18.4, 18.13). The owner can re-key the entry to `{ player: "<the tell>", from }` or `{ from }`.
...
- situations/probe-fernkampf.yaml 21.6 (Task 36: every remaining mismatch; was listed for the `process` key, which Task 28 defined): (a) steps 1–3 `process: { zielen: 2 }`, `{ zielen: 4 }`, `{ zielen: 4, capped: true }` get "none running", and the shot in step 4 has no fernkampf.FK11 line (+4, total 4 expected, 0 got) and no process to end: FK11's `process` rests on the open ruling fernkampf.zielen-interrupted, so it applies nothing and Zielen never starts; the offer of `zielen` and its Aktion match. (b) The numbers 2 / 4 / 4 are the bonus on the shot, not the progress: `process: { <id>: n }` compares the steps taken (Task 28, deviation 9: "Jede zum Zielen genutzte Aktion erleichtert den folgenden Schuss um 2 … bis zu einem Maximum von 4" is `steps: 2`, `2 per process.zielen`), which are 1 / 2 / 2. (c) Step 4's `process: { zielen: ended }`: the step states no die (`rolls: { roll.attack: n }`), so the shot's combat roll stays at its target stage and never states `action.attack: hit | miss`, which FK11's `breaksOff` reads (the page: Zielen lasts "bis zum eigentlichen Angriff"); the events of a roll come with its die (spec §6), and no source states the die (R61). The situation-level `texts: [{ ruling: fernkampf.zielen-interrupted }]` matches. In a throwaway run with zielen-interrupted decided, (a) is gone: Zielen runs 1 / 2 / 2, the third action is capped, and the shot reads FK11's +4, total 4 (matches); (b) and (c) remain, and the expected open-ruling text is then rightly missing. The owner re-keys the numbers to the progress (1 / 2 / 2) or to the FK line, and gives the shot a die. Task 28 named this for the owner (its report, concern 1).
...
### (b) Input convention: the sheet base excludes item and shield modifiers (R66)

The base contract (ADR-0015; `Situation.base` in Packages/RulesEngine): a sheet base handed to
the engine is the unfolded base, the technique or sheet value without the item's and the
shield's modifiers, which the engine adds as lines after it (at-pa-modifikatoren.M1: "erst nach
der Ermittlung der Basiswerte"; schilde.SCH1, SCH3; ruling R66). These situations state folded
bases, with those modifiers already counted in, so the modifiers count twice. By ruling R77 the
bases are not restated in this branch: the owner restates each from the file header's own
derivation (boronmir-sf and boronmir-neu: "PA 8, with the Großschild's passive +3: 11" and
"PA 7 + 2 × 3 = 13"; kampfsituationen: "shield parry ⌈10/2⌉ + 2 + 2×3 = 13", "Rabenschnabel …
7 + 2 − 1 = 8", "shield AT 10 + 2 − 6 = 6"; reiterkampf's generic `pa: 8`). No `expect` changes.
...
The situation expects what a rule would do that no rule file encodes (the page is not drafted,
or the rule is out of scope of its file). The owner writes the rule data, or drops the
expectation.
...
- situations/kampfwerte.yaml 16.20: expects `events: [{ gained: imSterben, from: schaden.S8 }]`. There is no rule `imSterben` and the Regel-Wiki has no page to draft one from; schaden.S8 now `tell`s the hero "Ab 0 Lebenspunkten liegt ein Held im Sterben." The harness also records `leCurrent: { result: -2 }`, got 4 (final review): −2 is 4 − 6, the hit's TP 6 taken whole as SP, which needs an RS of 0. The situation states no armour (no `loadout.armour`, no RS), so the hit's `rs` and `sp` stages ask for `loadout.armour` and stay unknown, and LE stays at its sheet value 4. No source states that the hero wears none, so the input is not filled in (R61: an unstated input is an expectation conflict). The comment's "or 0" is the question the missing Sterben rule would answer.
...
- zaubermodifikationen.ZM1 (Task 35): the offer's new `labels` name the seven options by ZM11's overview ("Erzwingen", "Kosten senken", …; its "Geste oder Formel weglassen" is two options, "Geste weglassen" and "Formel weglassen", as the app's two toggles); each ZM11 line that reads one option carries its label as the line's `term` (probe-magie 20.2–20.5). Vocabulary: the offer's optional field `labels` (field type `labels`, an option → a non-empty name; rulec checks each key is an option, tests `test_an_offer_names_its_options_with_labels`, `test_an_offers_labels_name_its_options_with_text`)
- zaubermodifikationen.ZM12 (Task 35): the failure cost is two effects: for an aufrechterhaltend spell half of `spell.cost` + `spell.costPerInterval` (as before), for any other spell half of `spell.cost`. The one amount read `spell.costPerInterval` for every spell, which only ZM5 derives (for an aufrechterhaltend spell), so for a sofort spell the value could not be computed and the failed cast paid nothing (probe-magie 20.3); the comment's intent ("for a spell that is not aufrechterhaltend … it adds nothing") is now the encoding. The half, rounded up, and the Ignifaxius numbers are unchanged
- fernkampf.FK11 (Task 36 fix round 1, ruling R74): Zielen's `breaksOff` reads `action.attack: [hit, miss, confirmedFumble]` (was `[hit, miss]`): a confirmed Patzer is the shot "bis zum eigentlichen Angriff" too (FK14: "gewöhnliches Misslingen" plus SP)
...
- Two open rulings placed wider than their question (Task 35, probe-magie; not changed, the owner's placement): zaubermodifikationen.cost-off-table asks what a step does to a cost *off* ZM8's table, but it sits on ZM11's two cost steps for every cost, so Ignifaxius's 8 → 16 (Erzwingen, 20.2, 20.4), 8 → 4 (20.3) and Flim Flam's 2 → 1 (20.5), all on the table, wait on it too; SA_74.vp-sequence asks when the Selbstbeherrschung is rolled, but it also sits on VP1's offer of the LeP split, which both of its options make before the roll (20.7 expects the offer with no ruling). Moving either ruling off those effects would let 20.2, 20.3 and 20.5 pass (a throwaway run with cost-off-table decided showed it) and 20.7's offer match; it needs the owner's word, since the rule files are theirs
- probe-fernkampf 21.1, 21.2 (Task 36; listed under the conflicts until Task 36, for expectations the migration turned into comments): the old situations expected the range band a distance gives (`range_band: { value: nah, from: fernkampf.FK4, ruling: fernkampf.range-input }`, `{ value: weit, from: fernkampf.FK4 }`). No expect key states a fact's value, and while ruling range-input is open the band is stated, not derived from the distance: the band is now input (`target.rangeBand`, with `target.distance` kept) and the expectation a comment. Only the `from: fernkampf.FK4` is lost: range-input stays cited, on 21.1's FK5 lines (FK and TP), which rest on it (FK5's adds read the band). 21.1 also had `on_hit: { tp: { formula: "1W6+4 +1" } }`: the TP formula is a comment (as reiterkampf.yaml); the query `tp(with: Kurzbogen)` keeps its total and line. Neither is a mismatch: nothing compares a comment. What mismatches is fernkampf.FK5's two adds (FK +2 / −2, TP +1 / −1), which rest on the open ruling range-input and so apply nothing; in a throwaway run with range-input decided, both situations pass. So they are pending on range-input, not conflicts. Open: once range-input is decided, should 21.1 and 21.2 get their band back as an expectation (option a derives it from `target.distance` through FK4's table, which then joins `via`)?
- fernkampf.FK8's term (Task 36): 21.3 expects the replaced FK6 line with `term: "Deckung: gilt als klein"` (Notes for the engine tasks, Task 22: the term names the row the replacement reads, `target.sizeInCover: klein`). The engine names a line's term only from an offer's `labels` (Task 35), and nothing gives FK8's `replace` a name: FK8's `ask` of `target.cover` has no labels and its `when` reads two options. While cover-as-size is open this is hidden (the line is not there); with it decided, 21.3 would stop on the unsupported shape `term` (a throwaway run shows it). The wording is the ruling's own (option a: "the line says 'Deckung: gilt als …'"; option b names the visible part instead), so it is left until the ruling is decided. Proposal: `replace` takes optional `labels`, keyed by the rows of the table its `with` reads (`klein: "Deckung: gilt als klein"`, …), and the replaced line's term is the label of the row read, as an offer's `labels` name its options.
- `action.attack: confirmedFumble` (Task 36, fixed in fix round 1 by ruling R74: a confirmed Patzer is a failed attack that was made): fernkampf.FK11's and ladezeiten.LZ2's `breaksOff`, LZ2's emptying and LZ7's spent ammunition read `[hit, miss, confirmedFumble]`, SA_62.ST3's and SA_172.U2's Passierschlag `[miss, confirmedFumble]` (see "Reviews reset by hand edits"). Left for the owner: with the optional Fernkampf-Patzertabelle (fernkampf.FK14's comment; the app has it, FumbleTable) in play, does a table result such as a Ladehemmung or "Kamerad getroffen" replace these consequences of a Patzer (the weapon emptied, the arrow spent, Zielen ended), or come on top of them?
...
- probe-magie 20.2–20.5 (Task 22/24): the line key `term` names the modification a ZM11 line belongs to ("Erzwingen", "Zauberdauer erhöhen"), since one clause gives several lines. The engine takes it from the choice option the line's `when` read (`choice.spellModification.erzwingen`); the display names of the options (lowerCamelCase ids) must come from somewhere, e.g. the picker's labels. Task 35: the offer's `labels` (zaubermodifikationen.ZM1) are those names; a line whose `when` reads exactly one labelled option (`choice.X.Y`, or `choice.X` compared with Y) carries its label, and the harness matches `term` as one of a line's fields, so ZM11's two lines of one value pair by it.
...
- probe-magie 20.7, 20.8 (Task 35): a cast (`.cast`) asks the talent checks whose `when` reads what it states (`check.kind`, `check.spell`, the chosen modifications), as `.take` asks those gated on the choice taken: SA_74.VP2's Selbstbeherrschung (an open ruling asks nothing and shows its text). A forbid from a check's `onFailure` carries the decided rulings of the check that asked it (as R68's suppressor). The harness compares a cast step's `result` with the spell check's result, run as the step states it: one `check.result` for every check (20.7's note above), a failure fails the checks the cast asked, and their forbids fail the cast (`.forbidden`), named by the forbid and resting on its rulings
