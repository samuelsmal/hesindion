...
conflicts for the owner" below, with the reasoning and each remaining mismatch.
...
- `action.attack: confirmedFumble` (Task 36): the combat roll states a confirmed Patzer of an attack as `action.attack: confirmedFumble` in place of `miss` (ITEMTPL_19's Fokus clause reads it). Every other reader of an attack's end reads `miss` or `[hit, miss]`: fernkampf.FK11's and ladezeiten.LZ2's `breaksOff`, LZ2's "the shot empties the weapon", LZ7's spent Pfeil or Bolzen, SA_62.ST3's and SA_172's Passierschlag after a failed attack. So after a confirmed Patzer Zielen keeps running, the weapon stays loaded, no ammunition is spent and a failed Sturmangriff gives no Passierschlag. No situation reaches it. Proposal: those `when`s read `[hit, miss, confirmedFumble]` / `[miss, confirmedFumble]` (the Patzer is a failed attack too), each a hand edit with `reviewed: null`; or the owner says a Patzer's consequences replace them.
