...
- situations/kupperus-und-waffen.yaml 18.1, 18.5, 18.11: every `formula:` (a TP formula or a computation, e.g. "2 + 12/2", "1W6+4 +9", "−⌈(25 − 20)/2⌉") is a comment: no line or query key holds a formula.
...
- situations/kampfsituationen.yaml 17.22 (Task 31, was listed for the `raises` key, which is a comment): expects `texts: []` ("no defence restriction"). `[]` asserts that no `tell` is shown, but every query shows the `*` tells that hold: kampfsonderfertigkeiten.KS2's "passiv — wirkt immer" and regeneration.R7's "Heilkräuter wirken dennoch." The lines and total match.
...
- situations/beidhaendiger-kampf.yaml 11.10 (Task 31): expects the loadout entry `{ other: Holzschild, allowed: true }`. `loadout.other` is the kind of the second piece (weapon, Parierwaffe or shield: the file's header), so the entry states `loadout.other: Holzschild`, which beidhaendiger-kampf.ZW1 (`not: { loadout.other: shield }`) reads as no shield: with the Morgenstern's Kettenwaffen (R61) it is forbidden like the Dolch. Nothing states that the Holzschild is a shield (no equipment rule describes it). The Dolch entry (`allowed: false, because: ZW1`) matches.
...
- situations/kampfsituationen.yaml 17.4 (Task 31): expects trefferzonen.TZ5's −10 line to carry the ruling passierschlag.passierschlag-zone. That ruling sits on passierschlag.PS3's offer of the zone (`appliesTo: [PS3]`); the line is TZ5's add, which rests on no ruling. A line carries its own effect's rulings (and a useLevel's that changed its rule's Stufe), not those of the offer whose choice its `when` read.
- situations/reichweite.yaml RW.16 (Task 31): expects the unterlaufen offer to carry the ruling reiterkampf.mounted-manoeuvres. The offer (SA_172.U2) rests on sf-technique-lists; mounted-manoeuvres sits on reiterkampf.RK3's forbid of Spezialmanöver, which does not name a Basismanöver and so neither refuses nor touches the offer. `kind: basismanoever` and the AT lines match.
- situations/reiterkampf.yaml 5.5 (Task 31): expects `itemChanged: loadout.mount, change: { ridden: false }` from reiterkampf.RK6. RK6's `item` changes the instance in the slot `mount` (`loadout.mount.instance`), which the file does not state (it states `hero.mounted`, not a mount in the loadout; `loadout.mount` is no vocabulary fact), so the change has no instance and the engine asks for it. Taking the jump off is run (the offer, the AW 0 match).
