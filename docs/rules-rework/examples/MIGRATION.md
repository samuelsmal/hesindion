...
- situations/boronmir-neu.yaml 19.12: expects `events: [{ logged: "Autoritätsglaube: gibt der Schlechten Eigenschaft nach", from: DISADV_37.SE2 }]`. SE2 is a `tell` to the player on the failed Willenskraft check: its text ("ansonsten muss er entsprechend der Schlechten Eigenschaft agieren") is roleplay and names no record, and no verb yields a `logged` event (plan Task 26 has the check procedure log "gibt nach" at `.confirm`). The rules give a text, not an event. The owner decides whether the procedure logs a `tell` that fires at `.confirm` (then SE2's text is the entry and the expectation stands) or the expectation becomes a `texts` entry.
...
- situations/kampfsituationen.yaml 17.7: expects `counts_as_defence: false` (a Passierschlag does not move mehrfache-verteidigung). No expect key says that a fact did not change; it is a comment. `actions_used: 0` is `events: []`.
- situations/kampfsituationen.yaml 17.8: expects `next: takeDamage` (the take-damage screen follows). No expect key names the next screen; it is a comment.
...
- situations/lebensenergie.yaml 15.7 (Task 30; 15.4–15.6 and 15.8 now pass: a line of a rule whose level a useLevel changed carries that useLevel's rulings, the base line of `level(rule: COND_6)` included, and `hero.conditionLevels` is derived): the step after the passed Selbstbeherrschung check expects `at` and `gs` at −3 (the hero acts at Schmerz III). The top-level expectation has the hero gain Handlungsunfähig (STATE_8, from COND_6.SZ5 at Stufe IV), and no rule clears STATE_8 when the check is passed: ADV_49.ZH3 only keeps or lowers the Stufe the effects read, and STATE_8.H2's `set` of GS 0 stays (gs −11 = −3 − 8). Whether passing the check ends the Handlungsunfähigkeit it prevents (an effect `gain: { rule: STATE_8, levels: -1 }` on the passed check), or the step describes a hero who never lost it, is the owner's call. The `offered` check entry (`before: each_action`) is also no choice: an offer offers choices, COND_6.SZ2's check is asked, not offered.
- situations/kampfwerte.yaml 16.1 (Task 30): expects the Leiteigenschaft term of `pa(with: Hiebwaffen)` `via: [kampfwerte.KW6]` and of `pa(with: Schwerter)` `via: [kampfwerte.KW6, kampfwerte.KW7]`. The numbers match (7 + 2, 6 + 2). The rules resolve `technique.leit` from KW21's table (`kampfwerte.leiteigenschaft`, `readBy: loadout`; KW6's own comment: "The PA half (`technique.leit`) is defined on KW21"), so the line's `via` is KW21; KW7 (pick the higher of two) is a `none` clause folded into how the loadout resolves the fact, and no effect can put it in a `via`. The owner decides whether the table moves to KW6 (the Kodex's PA rows) and KW7 gets an effect, or the expectation names KW21.
...
- situations/verweichlicht.yaml 13.4 (Task 30): expects on the hero sheet `notApplied: [{ rule: DISADV_57, reason: needs the Trefferzonen Fokusregel, ruling: DISADV_57.verweichlicht-scope }]`, with the Fokusregel off. The sheet (`Engine.sheet(in:)`, the breakdown every rule's `*` effects reach) holds no DISADV_57 entry: VW1's only effect is an `add` to `check.modifier` gated on the Wundeffekt check (`gmFact.cause: woundEffect`), which only the Fokusregel asks for; no effect says the disadvantage needs the Fokusregel, so no breakdown can list it as not applied for that reason. `events: []` (no check asked) matches. The owner decides whether DISADV_57 gets a rule-level statement of its ruleset (a `tell` or a `require` on `rulesets`) the sheet can show.
