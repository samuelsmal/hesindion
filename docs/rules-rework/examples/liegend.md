...
- **L3** Their defence is −2, their attacks −4.
...
counts as Liegend; the GM may allow free actions to speak.
...
Where the app is wrong: a Handlungsunfähig hero moves at GS 1 instead of 0, because
`Hero.effectiveGeschwindigkeit` only knows Liegend (7.4); a hero made Handlungsunfähig by a Zustand
at Stufe IV gets neither the implied Liegend nor a lower GS, because `Hero.impliedStateIDs` reads
only states that are set, not `isHandlungsunfaehigFromZustaende` (7.5); there is no stand-up action,
