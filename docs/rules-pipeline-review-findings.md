...
Orienting reading, if you have not seen this branch: `docs/rules-pipeline-status.md` (§1, §6, §7,
§8), `docs/plans/2026-09-20-rules-pipeline-and-authoring.md`, ADRs `0007`/`0008`/`0009`, `AGENTS.md`.
...
- **Finding 2** is verbatim German rule prose committed to `CHANGELOG.md`, to
  `docs/rules-pipeline-status.md` — in a file that opens by asserting it contains none — and to
  `docs/adr/0008`. The Data Policy in `AGENTS.md` names documentation explicitly, and
...
That is real value and it is correctly scoped. But no guarantee in ADR-0008 about the *engine* is
...
- `docs/adr/0008-rules-as-data-combat-engine.md:192-196` — states `SA_661`'s encoded row in full:
  effect type, target, scope, value, and both `when` predicates including the `gmFlag` slug.
- `docs/adr/0008-rules-as-data-combat-engine.md:61` — states `SA_48`'s Stufe II opponent-side value
  as a quoted German display string.
- `docs/adr/0008-rules-as-data-combat-engine.md:64` — states `SA_59`'s `dice.recipient`.
- `docs/adr/0008-rules-as-data-combat-engine.md:125` — states the mounted BE row's value and gate.
- `docs/adr/0009-rule-provenance-and-rule-sets.md:15` — states `CHAP_Reiterkampf` clause 6's value.
...
`docs/rules-pipeline-status.md` §2 itself says, "redaction removes the identifying token, not the
...
workspace for precedent, finds `docs/adr/0008-rules-as-data-combat-engine.md`, and reads a sentence
...
`docs/rules-pipeline-status.md` §2) calls **evidence of contamination, not success** — and there is
...
its failure is one of the three the five blockers are derived from. ADR-0009:13 now gives it away.

**Why per-task review could not see it.** ADR-0009 did not exist and ADR-0008's 2026-09-21
amendments were not written when the Task 6 leak audit ran. That audit recorded exactly two residual
leaks (`AGENTS.md`'s unattributed Belastung sentence, and ADR-0008's `CheckDomain` paragraph) and
...
Update `docs/rules-pipeline-status.md` §2's residual-leak list either way — its "two residual leaks
...
- `CHANGELOG.md:94` — a full clause of `SA_661`'s page quoted verbatim, twelve words, in quotation
  marks and italics.
- `docs/rules-pipeline-status.md:200` — the same clause, same quotation. This file's line 20 reads
  "No DSA rule prose appears here, per the Data Policy in `AGENTS.md`."
- `docs/adr/0008-rules-as-data-combat-engine.md:100-101` — two maneuver-exclusion clauses quoted
  verbatim, seven and eight words.
- `docs/adr/0008-rules-as-data-combat-engine.md:61` and `:66` — two shorter quoted German fragments
  of rule text.
...
This is an improvement and it is the right rule — ADR-0008 argues it well, and the old predicate
...
and ADR-0008. (a) is maybe twenty lines and matches the precedent. Either way it needs to be a
...
**Severity: important. File it — it is a live instance of the pattern ADR-0007 exists to remove.**
...
**Why it matters.** ADR-0007's founding argument is "two authorities, no arbiter" — the bug the
...
line in ADR-0008's Consequences saying the engine wiring must delete the hand-written definition in
...
`docs/rules-pipeline-status.md` §7; I am ruling it a must-schedule rather than a nice-to-have, for
the reason below.
...
**Concrete failure scenario.** ADR-0008 plans a named-DSA-constants registry; call the file
...
either. The knowledge lives in `AGENTS.md` and `docs/rules-pipeline-status.md` §1 and §3 — both
...
`CALIBRATION GATE: FAILED — Tier 1 7/10 — no authoring wave (docs/rules-pipeline-status.md §3)`;
...
`python3 -m pytest tests/ -q`, which is the command `docs/rules-pipeline-status.md` §9 and
...
The list parked in `docs/rules-pipeline-status.md` §7, ruled one by one. Numbering follows the
review brief.
...
3. **ADR-0008 still records it as a binding input to the engine plan**
   (`docs/adr/0008-rules-as-data-combat-engine.md:158-165`), and the amendment is intact and
...
not one — a line in ADR-0008's existing consequence paragraph is enough.
...
- The three authorised Swift behaviour changes, read against their authored rule files and the
  reasoning in `CHANGELOG.md` / `docs/rules-pipeline-status.md` §5 and §6(b). I did **not** fetch
...
`0d55ce3..1c231dd` — the range the plan's status board carries; this line cited the pre-fix-round
