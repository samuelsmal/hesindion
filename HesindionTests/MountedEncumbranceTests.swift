...
///
/// The Reiterkampf chapter eases Kampfproben, and Kampf and Zaubern are neither the same
/// nor related: only a Kampfprobe is eased. The
...
final class MountedEncumbranceTests: XCTestCase {
    ...
    private static let combatDomains: [RuleDomain] =
        [.meleeAttack, .meleeParry, .meleeDodge, .rangedAttack]
    private static let castingDomains: [RuleDomain] = [.spellCasting, .liturgyCasting]
    ...
    private func encumbrance(_ hero: Hero, _ domain: RuleDomain, mounted: Bool) -> Int {
        var situation = Situation(hero: hero, domain: domain)
        situation.round.mounted = mounted
        return ModifierEngine.shared
            .evaluate(context: situation)
