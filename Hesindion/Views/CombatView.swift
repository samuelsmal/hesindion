...
enum CombatStep {
    ...
    case execution(CombatAction, name: String, attributeValue: Int, damageFormula: String?, note: String?, modifierLines: [ModifierLine]? = nil, secondAttack: (name: String, at: Int, damage: String?)? = nil, damageLines: [ModifierLine] = [], damageMultiplier: CriticalDamage = .unchanged)
    ...
    case takeDamage
    case flucht
    case opponentDefense(weaponName: String, damageFormula: String?, isCriticalHit: Bool, criticalDamage: CriticalDamage, modifierLines: [ModifierLine]?, isRangedAttack: Bool = false, rangedDefensePenalty: Int = 0, damageLines: [ModifierLine] = [], damageMultiplier: CriticalDamage = .unchanged)
    ...
    case criticalSuccess(table: CriticalSuccessTableType?, action: CombatAction, weaponName: String, damageFormula: String?, modifierLines: [ModifierLine]?, isRangedAttack: Bool = false, rangedDefensePenalty: Int = 0, damageLines: [ModifierLine] = [], damageMultiplier: CriticalDamage = .unchanged)
    case passierschlag
    case fernkampfSetup
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            case .announcement(let action, let name, let baseAT, let dmgFormula, let isOffHand, let secondAttack, let isMountCharge):
                ...
                .transition(.move(edge: .trailing))
            case .execution(let action, let name, let attrValue, let dmgFormula, let note, let modifierLines, let secondAttack, let damageLines, let damageMultiplier):
                CombatExecutionView(
                    hero: hero,
                    action: action,
                    weaponName: name,
            ...
            case .takeDamage:
                CombatTakeDamageView(hero: hero, step: $step, onDismiss: onDismiss, combatId: combatId, roundNumber: roundNumber)
                    .transition(.move(edge: .trailing))
            case .opponentDefense(let name, let dmg, let isCrit, let criticalDamage, let mods, let isRanged, let rangedPenalty, let damageLines, let damageMultiplier):
            ...
            case .fumbleChoice(let action, let name, let isShield):
                ...
                .transition(.move(edge: .trailing))
            case .criticalSuccess(let table, let action, let name, let dmg, let mods, let isRanged, let rangedPenalty, let damageLines, let damageMultiplier):
